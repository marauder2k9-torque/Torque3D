...
void GFXTextureArray::setTexture(const GFXTexHandle& texture, U32 slot)
{
   ...
   if (texture->getPath().isNotEmpty())
   {
      if (texture.getHeight() != mHeight || texture.getWidth() != mWidth || texture.getFormat() != mFormat || texture->getMipLevels() < mMipLevels)
      {
         ...
         {
            AssertFatal(true, avar("GFXTextureArray::setTexture all textures must be convertible to GFXFormat %d", mFormat));
            Con::errorf("GFXTextureArray::setTexture all textures must be convertible to GFXFormat %d", mFormat);
            handle = NULL;
            delete inBitmap;
         }
         else
