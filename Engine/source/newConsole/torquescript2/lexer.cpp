...
namespace
{
   /// Mirrors charConv (CMDscan.l) exactly: r/n/t map to their control
   /// characters, everything else passes through unchanged (backslash
   /// dropped by the caller, this only resolves the character after it).
...
void Lexer::skipWhitespaceAndComments()
{
   for (;;)
   {
      ...
      // "///" doc-block lines are handled by scanDocBlock, called
      // separately from next() before this loop ever sees them - by the
      // time we're here, a leading "///" has already been ruled out by
      // the caller checking peek(0..2) first. Plain "//" line comments
      // and "/* */" block comments are consumed here.
      if (c == '/' && peek(1) == '/' && peek(2) != '/')
      {
         while (!isAtEnd() && peek() != '\n')
            advance();
...
Token Lexer::scanIdentifierOrKeyword()
{
   ...
   // "true"/"false" lex directly as IntConst - CMDscan.l has no
   // boolean-literal token, these are int literals by rule.
   if (text.equal("true"))
   {
   ...
   // Reserved words take priority over {ID} - checked before the
   // TypeNameLookup callback, matching CMDscan.l's rule ordering.
   ...
   // Sc_ScanIdent's live registry check: an identifier matching a
   // registered engine type name lexes as TypeIdent, not Ident.
...
Token Lexer::scanVar()
{
   ...
   advance(); // sigil ($ or %)
   advance(); // mandatory LETTER after the sigil - caller (next()) has
              // already verified peek(1) is a letter before calling this

   // VARTAIL = {VARMID}*{IDTAIL} where VARMID includes ':' but IDTAIL
   // does not - so a trailing run of colons is not consumed here, only
   // a colon run followed by at least one more [A-Za-z0-9_] is. Scan
   // greedily allowing ':', then trim back if we ended on one or more
...
Token Lexer::scanNumber()
{
   ...
   // Consume a leading digit run (may be empty, for the ".5" float shape).
   while (!isAtEnd() && isdigit((unsigned char)peek()))
      advance();

   bool isFloat = false;

   // FLOAT alternative 1: {INTEGER}?\.{INTEGER} - dot must be followed
   // by at least one digit, or this is not that alternative at all (a
   // trailing "5." with nothing after the dot does not match FLOAT in
   // the original grammar - see tokenKinds.inc / lexer.h notes).
   ...
   // FLOAT alternative 2: {INTEGER}(\.{INTEGER})?[eE][+-]?{INTEGER} -
   // exponent is mandatory for this alternative. Only reachable here if
   // alternative 1 did not already consume a dot (the grammar's two
   // alternatives are mutually exclusive on which one a given lexeme
   // matches, but a mantissa with a dot AND an exponent, e.g. "1.5e5",
   // is still alternative 2 with an optional dot inside it - so this
   // check must still fire even after alternative 1's dot was consumed).
   if ((peek() == 'e' || peek() == 'E') &&
...
{
   // Mirrors collapseEscape (CMDscan.l) exactly: \x + 2 hex digits, \c +
   // digit/r/p/o with the specific remap table, otherwise \X -> charConv(X)
   // (r/n/t -> control chars, anything else passes through literally
   ...
   while (p < rawEnd && outLen < sizeof(buffer) - 1)
   {
      if (*p == '\\' && p + 1 < rawEnd)
      {
         char next = p[1];
         if (next == 'x' && p + 3 < rawEnd)
         {
            ...
            if (d1 < 0 || d2 < 0)
               return false;
            buffer[outLen++] = static_cast<char>(d1 * 16 + d2);
            p += 4;
         }
         else if (next == 'c' && p + 2 < rawEnd)
         {
            ...
            // "Make sure we don't put 0x1 at the beginning of the
            // string" - collapseEscape's own guard, reproduced exactly:
            // if this would be the first output byte and equals 0x1,
            // emit 0x2 followed by 0x1 instead of a bare 0x1.
            if (outByte == 0x1 && outLen == 0)
            {
               buffer[outLen++] = static_cast<char>(0x2);
               if (outLen < sizeof(buffer) - 1)
                  buffer[outLen++] = static_cast<char>(0x1);
            }
            else
            {
               buffer[outLen++] = static_cast<char>(outByte);
            }
            p += 3;
         }
         else
         {
            buffer[outLen++] = static_cast<char>(charConvEscape(next));
            p += 2;
         }
      }
      else
...
Token Lexer::scanDocBlock()
{
   ...
   // (\"///\"([^/\n\r][^\n\r]*)?[\n\r]+)+ - one or more consecutive
   // "///"-prefixed lines. Strip the leading "///" from each line and
   // any \r, join the remainder with newlines already present in source.
   for (;;)
   {
...
{
   // Ordered/matched exactly against CMDscan.l's reserved-word rules.
   // "namespace" and "class" are deliberately NOT here - see
   // CMDscan.l's own comment on why (removed from the grammar, no
   // productions ever consumed them).
...
Token Lexer::scanOperatorOrPunct()
{
   ...
   switch (c)
   {
      ...
      case '-':
         // Order matters: "-->" must be checked before "--", since
         // match('-') would otherwise greedily consume the second '-'
         // and never let the three-char form be recognized.
      ...
      case '$':
         ...
         // A bare '$' not followed by a LETTER (i.e. not a VAR) has no
         // production in CMDscan.l's rule set shown - treated as
...
Token Lexer::next()
{
   // "///"-doc-blocks must be checked before the generic "//"/"/*"
   // comment skip, since skipWhitespaceAndComments treats "///" as
   // ordinary whitespace-adjacent text it does not specially recognize -
   // matches CMDscan.l's rule ordering (doc-block rule listed first).
   ...
   {
      // "NL"/"TAB"/"SPC" are lexed as identifiers by the generic {ID}
      // shape, but CMDscan.l gives them dedicated rules ahead of {ID}
      // that all `return '@'` with a separator-char payload - reproduce
      // that priority here before falling into the general identifier
      // path.
